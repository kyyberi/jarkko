...
test("server-renders the professional homepage", async () => {
  ...
  assert.doesNotMatch(css, /\.work-row/);
  ...
  assert.match(html, /href="\/work\/government-ai"[\s\S]*Explore more/);
  assert.match(html, /Data Products at Scale/);
  assert.match(html, /href="\/work\/maysano"[\s\S]*Explore more/);
  assert.match(html, /Open standards and SDK/);
  assert.match(html, /href="\/services\/odps"[\s\S]*Explore more/);
  assert.doesNotMatch(html, /Explore standards and SDK/);
  assert.match(html, /Open Data Value/);
  assert.match(html, /href="\/work\/open-data-value"[\s\S]*Explore more/);
  ...
  assert.match(html, /href="\/work\/school-of-ai"[\s\S]*Explore more/);
...
test("server-renders insights report library with gated download controls", async () => {
  ...
  assert.match(css, /\.insights-library-head\s*\{[^}]*grid-template-columns:\s*minmax\(0, 0\.82fr\) minmax\(320px, 0\.72fr\);/);
  assert.match(css, /\.insights-library-visual\s*\{[^}]*min-height:\s*250px;/);
